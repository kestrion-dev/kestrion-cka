...
access: free
updated: "2026-09-08"
prerequisites: [arquitectura-kubernetes]
---
...
De [M02 (Arquitectura)](/modulos/arquitectura-kubernetes/) usarás la regla de que solo kube-apiserver habla directamente con etcd, y el concepto de que etcd es un almacén distribuido y consistente, la base para entender el quorum de esta sección.

De [M03 (kubeadm)](/modulos/kubeadm-etcd-backup-restore-upgrade/) usarás el mecanismo interno de `kubeadm init` y `kubeadm join`, ya que HA reutiliza ambos comandos con flags adicionales, sin cambiar su lógica de fondo.
...
Esta regla de número impar es estrictamente del quorum de **etcd**, no de "control planes" en general. En la topología stacked, que ves en 4.3, cada control plane aloja también un miembro de etcd, así que ambos números quedan atados y el control plane hereda indirectamente la necesidad de ser impar. En la topología external, en cambio, los apiservers no forman ningún quorum entre sí: solo el clúster de etcd separado necesita un número impar de miembros; el número de apiservers puede ser cualquier cantidad igual o mayor a 3.

### 4.3 Las dos topologías

#### Etcd apilado, stacked etcd

Es la topología por defecto de kubeadm. etcd corre como static pod DENTRO de cada nodo de control plane, exactamente igual que el `etcd-cka-cp1` que ya conoces de M02 y M03, pero replicado en cada uno de los 3 nodos. Cada control plane tiene su propio miembro etcd local. Es más simple de operar y requiere menos máquinas DEDICADAS a control plane/etcd, 3 en vez de 6, pero acopla los fallos: perder un nodo de control plane significa perder también un miembro de etcd al mismo tiempo. Contando también los workers, un clúster stacked completo necesita un mínimo de 6 máquinas: 3 de control plane y 3 workers.

#### Etcd externo

etcd corre en su propio cluster de 3 nodos, completamente separado de los nodos de control plane. Desacopla los fallos: perder un control plane no afecta a etcd. Requiere el doble de máquinas DEDICADAS a control plane/etcd como mínimo frente a stacked, 3 para el control plane y 3 para etcd, 6 en total. Contando también los workers, un clúster external completo necesita un mínimo de 9 máquinas: 3 de control plane, 3 de etcd y 3 workers.
