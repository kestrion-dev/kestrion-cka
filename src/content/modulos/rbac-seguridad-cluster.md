...
De [M02 (Arquitectura)](/modulos/arquitectura-kubernetes/) usarás el flujo de una petición a la API, autenticación, autorización, admisión, que aquí se profundiza específicamente en la fase de autorización.

De [M03 (kubeadm)](/modulos/kubeadm-etcd-backup-restore-upgrade/) usarás la CA del cluster en `/etc/kubernetes/pki/` y la estructura de un kubeconfig, ambas necesarias para construir la identidad de un usuario nuevo.
