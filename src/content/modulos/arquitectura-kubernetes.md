...
De [M01](/modulos/entorno-laboratorio-kubectl/) usarás la familiaridad con kubectl en modo imperativo y con `kubectl describe`, ya que este módulo se apoya en observar el cluster real para ilustrar cada componente, en lugar de quedarse solo en la teoría.
...
Corre en cada nodo y mantiene las reglas de red que permiten que el tráfico dirigido a un Service llegue a los pods correctos, sin importar en qué nodo estén. Puede operar en modo `iptables`, `ipvs` o `nftables`; `ipvs` está deprecado desde la versión 1.35, y `nftables` es estable desde la 1.33. kube-proxy es además un componente opcional: un cluster puede sustituirlo por una alternativa de red que implemente el mismo comportamiento de Service. Profundizarás en su mecanismo exacto más adelante en el curso, en el módulo dedicado a Services.
...
Los componentes del control plane y los kubelets evitan sondear constantemente al apiserver preguntando "¿algo cambió?". En su lugar, abren una conexión de watch (list/watch) contra el apiserver sobre el tipo de recurso que les interesa, por ejemplo pods sin nodo asignado, en el caso del scheduler, y el apiserver les notifica en tiempo real cuando ocurre un cambio relevante. Esto no elimina toda forma de sondeo: kubelet ejecuta además ciclos periódicos de resincronización, y su PLEG sondea localmente al container runtime para detectar cambios que el runtime no notifica por sí solo. list/watch reduce drásticamente la necesidad de sondear la API, pero no sustituye por completo esos mecanismos locales.
...
Confirmar que kubelet corre como proceso del sistema operativo, no como pod, es clave: kubelet se ejecuta como daemon del propio nodo y, en una instalación kubeadm, es quien supervisa los static pods del control plane, sin depender de ser gestionado por sí mismo como un pod. El container runtime, containerd en tu cluster, también corre fuera de cualquier pod, como proceso del nodo.
...
Los valores exactos varían según el cluster: el conteo de pods de kube-system depende de los add-ons instalados, y el nodo asignado al pod de verificación depende de la decisión del scheduler en el momento de crearlo. Acepta cualquier entero positivo consistente con lo que observes en tu cluster, y cualquier nodo elegible en el mensaje `Successfully assigned`.

```text reference
<entero positivo: total de pods en kube-system>

cka-cp1

active

Successfully assigned default/verificacion-arquitectura to <nodo-elegido-por-el-scheduler>
```
...
```bash exec
kubectl get pod kube-apiserver-cka-cp1 -n kube-system -o jsonpath='{.metadata.annotations.kubernetes\.io/config\.mirror}'
```

```text reference
<hash-del-manifiesto-estatico>
```

La presencia de la anotación `kubernetes.io/config.mirror` es la prueba directa y específica de que este pod es un mirror pod de un static pod. Como observación adicional, su `ownerReference` apunta al Node, no a un ReplicaSet ni a un DaemonSet:
...
No es un fallo, es el comportamiento esperado: la anotación `kubernetes.io/config.mirror` confirma que es un mirror pod, y kubelet, que sigue leyendo el manifiesto real en `/etc/kubernetes/manifests/` de forma independiente, lo vuelve a crear casi de inmediato tras cualquier `kubectl delete`. Para modificar un componente del control plane hay que editar su manifiesto real en esa ruta, no el mirror pod visible vía kubectl. Este flujo completo se practica en M03.
...
**Alternativa K3s (plan B):** la arquitectura conceptual es la misma, ya que K3s sigue siendo Kubernetes, con los mismos componentes lógicos. La diferencia relevante es que K3s reemplaza etcd por SQLite por defecto como backend de almacenamiento, y distribuye y administra varios componentes del control plane, incluido kubelet, desde un binario compacto mediante los procesos `k3s server` y `k3s agent`, lo que hace mucho más difícil observarlos como procesos o pods separados que en una instalación kubeadm tradicional. Los ejemplos de la sección 6 y el laboratorio guiado de la sección 7, que dependen de ver estos componentes como static pods individuales, no se pueden replicar de la misma forma en K3s.
