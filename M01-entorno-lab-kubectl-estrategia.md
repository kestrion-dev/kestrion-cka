...
Cada Tab equivale a 2 espacios, el estándar de indentación en manifiestos de Kubernetes.
...
Ahorra unos 8 caracteres por comando. Con 200 o más comandos en 2 horas, el ahorro es real.
...
Elimina pods sin esperar el grace period de 30 segundos. Uso típico: `kubectl delete pod mi-pod $now`.
...
Modificar recursos existentes, dos vías. La rápida es la edición directa:

```bash exec
kubectl set image deployment/mi-app nginx=nginx:1.26
```

La segura es extraer, editar y aplicar:
...
```yaml config
# editar mi-app.yaml con vim antes de aplicar
```
...
El examen usa 6 clusters. Cada pregunta indica cluster y namespace. El error más común del examen es trabajar en el contexto o namespace equivocado.
...
El asterisco en `get-contexts` indica el contexto activo. En el examen, cada pregunta comienza con el comando exacto para cambiar de contexto. Ejecútalo siempre antes de cualquier otra cosa.
...
Datos del examen: 120 minutos, 15 a 20 tareas, aprobado con 66% o más, 6 clusters distintos, cada pregunta muestra su peso en porcentaje.
...
**Siguiente módulo:** M02 - Arquitectura de Kubernetes