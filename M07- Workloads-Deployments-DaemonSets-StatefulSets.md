# M07 - Workloads: Deployments, DaemonSets, StatefulSets
...
De M02 (Arquitectura) usarás el reconciliation loop del controller-manager, observar, comparar, actuar, aplicado ahora a ReplicaSets y a los controllers de DaemonSet y StatefulSet.

De M03 (kubeadm) usarás la diferencia entre static pods, gestionados por kubelet directamente, y pods gestionados por un controller vía la API.

De M05 (RBAC) usarás los ServiceAccounts, ya que los controllers internos de Kubernetes usan RBAC para operar sobre estos recursos.
...
kubectl rollout history deployment/api-gateway --no-headers 2>/dev/null | wc -l > /tmp/revisiones.txt
```
