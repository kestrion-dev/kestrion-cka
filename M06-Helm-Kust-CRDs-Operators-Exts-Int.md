# M06 - Helm, Kustomize, CRDs, Operators y Extension Interfaces
...
De M02 (Arquitectura) usarás el concepto de controller y el reconciliation loop, con kube-controller-manager como ejemplo nativo, la base para entender el patrón Operator de este módulo.

De M03 (kubeadm) usarás static pods, `kubectl apply -f`, e inspección de componentes vía crictl.

De M05 (RBAC) usarás `kubectl api-resources` para descubrir apiGroups, y RBAC en general, ya que los Operators necesitan permisos para operar sobre sus propios CRDs.
